...
namespace WholesomeTBCAIO.Rotations.Shaman
{
    public class ShamanRestoParty : Shaman
    {
        ...
        protected override void HealerCombat()
        {
            ...
            // RipTide
            List<AIOPartyMember> alliesNeedingRipTide = AIOParty.Group
                .FindAll(a => a.IsAlive && a.HealthPercent < settings.PartyRipTideThreshold)
                .OrderBy(a => a.HealthPercent)
                .ToList();
            if (alliesNeedingRipTide.Count > 0
                && cast.OnFocusUnit(Riptide, alliesNeedingRipTide[0]))
              return;
