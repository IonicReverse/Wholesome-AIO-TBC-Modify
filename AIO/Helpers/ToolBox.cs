...
namespace WholesomeTBCAIO.Helpers
{
    public class ToolBox
    {
        ...
        public static uint SpellIdByNameEx(string spellName)
        {
          if (!string.IsNullOrWhiteSpace(spellName))
            return Lua.LuaDoString<uint>(string.Format("local name, rank, icon, castTime, minRange, maxRange, spellId = GetSpellInfo('{0}'); return spellId;", spellName.Replace("'", "\\'")), string.Empty);
